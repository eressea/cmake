...
FUNCTION(DETERMINE_LUA_VERSION _file _varname)
  ...
  ELSE()
    ...
    RETURN()
  ENDIF()
  STRING(REGEX REPLACE "^.*LUA_VERSION_NUM[ \t]+([0-9])0?([.0-9]+).*$" "\\1.\\2" ${_varname} "${_file_content}")
  ...
  IF(${_varname} STREQUAL "${_file_content}")
    ...
    MESSAGE(STATUS "version ${_varname} -- ${_file_content}")
    IF(${_varname} STREQUAL "${_file_content}")
...
# Find Lua 5.2
FIND_PATH(LUA_5.2_INCLUDE_DIR lua.h
  PATHS $ENV{LUA_DIR}
  PATH_SUFFIXES include/lua52 include/lua5.2 include/lua include
)
IF(LUA_5.2_INCLUDE_DIR)
  DETERMINE_LUA_VERSION(${LUA_5.2_INCLUDE_DIR}/lua.h LUA_5.2_VERSION)
  COMPARE_VERSION_STRINGS("${LUA_5.2_VERSION}" "5.2" _version_compare TRUE)
  IF(NOT _version_compare EQUAL 0)
    # Incorrect version found, abort search
    SET(LUA_5.2_INCLUDE_DIR "LUA_5.2_INCLUDE_DIR-NOTFOUND" CACHE PATH "" FORCE)
  ELSE()
    FIND_LIBRARY(LUA_5.2_LIBRARY_OPTIMIZED
      NAMES lua52 lua5.2 lua
      PATHS $ENV{LUA_DIR}
      PATH_SUFFIXES lib64 lib
    )
    FIND_LIBRARY(LUA_5.2_LIBRARY_DEBUG
      NAMES lua52d lua52_d lua5.2d lua5.2_d luad lua_d
            lua52D lua52_D lua5.2D lua5.2_D luad lua_D
      PATHS $ENV{LUA_DIR}
      PATH_SUFFIXES lib64 lib
    )
    HANDLE_LIBRARY_TYPES(LUA_5.2)
    SET(LUA_5.2_LIBRARIES ${LUA_5.2_LIBRARY})
    IF(LUA_5.2_LIBRARIES)
      SET(LUA_5.2_FOUND TRUE)
    ENDIF()
  ENDIF(NOT _version_compare EQUAL 0)
ENDIF(LUA_5.2_INCLUDE_DIR)
...
ELSE()
  IF(LUA_5.2_FOUND) # Prefer version 5.2 if found
    SET(LUA_VERSION_SELECTION "5.2")
	ELSEIF(LUA_5.1_FOUND) # Prefer version 5.1 if found
...
MARK_AS_ADVANCED(
  ...
  LUA_5.2_INCLUDE_DIR
  LUA_5.2_LIBRARY_OPTIMIZED
  LUA_5.2_LIBRARY_DEBUG
  UNIX_MATH_LIBRARY
)
